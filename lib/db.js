// TODO: consider moving to https://github.com/grncdr/node-any-db
// and https://github.com/brianc/node-sql, or just direct driver use.

var Sequelize = require('sequelize');

function DB(options, callback) {
  var self = this;

  switch(options.type) {
    case DB.MYSQL:
      options.dialect = DB.MYSQL;
      options.port = options.port || 3306;
      break;
    case DB.POSTGRES:
      options.dialect = DB.POSTGRES;
      options.port = options.port || 5432;
      break;
    case DB.MARIADB:
      options.dialect = DB.MARIADB;
      options.port = options.port || 3306;
      break;
    case DB.SQLITE:
    /* falls through*/
    default:
      options.dialect = DB.SQLITE;
      options.storage = options.storage || ':memory:';
      break;
  }
  delete options.type;

  options.logging = options.logging || false;

  // MakeDrive username to segment filesystems
  this.user = options.user;

  options.db = options.db || {};
  try {
    var sequelize = this.sequelize = new Sequelize(options.db.name,
                                                   options.db.username,
                                                   options.db.password,
                                                   options);
    sequelize.authenticate().complete(function(err) {
      if(err) {
        return callback(err);
      }

      var Data = self.Data = sequelize.define('makedrive_data', {
        user: {
          type: Sequelize.STRING,
          primaryPath: true
        },
        path: {
          type: Sequelize.STRING,
          primaryPath: true
        },
        data: Sequelize.BLOB
      },
      {
        // We don't want updatedAt, createdAt
        timestamps: false
      });

      Data.sync().complete(callback);
    });
  } catch(err) {
    return callback(err);
  }
}

DB.prototype.find = function(path, callback) {
  var user = this.user;
  this.Data.find({
    where: {
      user: user,
      path: path
    }
  }).complete(function(err, record) {
    if(err) {
      return callback(err);
    }

    callback(null, record ? record.data : null);
  });
};

DB.prototype.createOrUpdate = function(path, data, callback) {
  var user = this.user;
  this.Data.create({
    user: user,
    path: path,
    data: data
  }).complete(callback);
};

DB.prototype.remove = function(path, callback) {
  var user = this.user;
  this.Data.find({
    where: {
      user: user,
      path: path
    }
  }).complete(function(err, record) {
    if(err) {
      return callback(err);
    }
    record.destroy().complete(callback);
  });
};

DB.prototype.clearAll = function(callback) {
  var user = this.user;
  this.Data.destroy({user: user}, {truncate: true}).complete(callback);
};

DB.MYSQL = 'mysql';
DB.SQLITE = 'sqlite',
DB.POSTGRES = 'postgres';
DB.MARIADB = 'mariadb';

module.exports = DB;
